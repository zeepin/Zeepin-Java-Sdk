...
//            prikey = "523c5fcf74823831756f0bcb3634234f10b3beb1c05595058534577752ad2d9f";
            ...
            if(sdk.getWalletMgr().getWallet().getIdentities().size() < 2){
                ...
                String txhash = sdk.nativevm().GId().sendRegister(identity,password,payerAcct,sdk.DEFAULT_GAS_LIMIT,0);

                Identity identity2 = sdk.getWalletMgr().createIdentity(password);
                String txhash2 = sdk.nativevm().GId().sendRegister(identity2,password,payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
                Thread.sleep(6000);
...
//            System.out.println("account:" + sdk.getConnect().getBalance(account.getAddressU160().toBase58()));
            //不会触发
            if(false){
                sdk.nativevm().zpt().sendTransfer(account,account9.getAddressU160().toBase58(),100000000,payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
                Thread.sleep(6000);
                System.out.println("account" + sdk.getConnect().getBalance(account.getAddressU160().toBase58()));
                System.out.println("account" + sdk.nativevm().gala().unboundGala(account.getAddressU160().toBase58()));
                sdk.nativevm().gala().withdrawGala(account,account9.getAddressU160().toBase58(),640000000000L,payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
            }
           //不会触发
            if(false){
...
//                System.out.println(obj);
                ...
                System.out.println(sdk.nativevm().GId().sendGetDDO(Common.didzpt+adminZPTIdAcct.getAddressU160().toBase58()));
            }
            List<Identity> dids = sdk.getWalletMgr().getWallet().getIdentities();
            Identity identity = dids.get(0);
            System.out.println("identity:" + identity.gid);

//            Identity adminGid = sdk.getWalletMgr().createIdentityFromPriKey(password,adminPrivateKey);
           
            Identity adminGid = sdk.getWalletMgr().getWallet().getIdentity(Common.didzpt+adminZPTIdAcct.getAddressU160().toBase58());
...
//                Identity adminGid = sdk.getWalletMgr().getWallet().getIdentity("did:zpt:ZazEvfQPcQ2GEFFPLF1ZLwQ7K5jDn81hve");
//                String txhash = sdk.nativevm().auth().assignFuncsToRole(adminGid.gid,password,adminGid.controls.get(0).getSalt(),1,contractAddr,"role",new String[]{"registerCandidate"},payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
                String txhash = sdk.nativevm().auth().assignGIdsToRole(adminGid.gid,password,adminGid.controls.get(0).getSalt(),1,contractAddr,"role",new String[]{identity.gid},payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
                Thread.sleep(6000);
                Object obj = sdk.getConnect().getSmartCodeEvent(txhash);
                System.out.println(obj);

//                Object obj = sdk.nativevm().auth().verifyToken(identity.gid,password,identity.controls.get(0).getSalt(),1,contractAddr,"registerCandidate");
//                System.out.println(obj);
     ...
     /*           sdk.getWalletMgr().importAccount("blDuHRtsfOGo9A79rxnJFo2iOMckxdFDfYe2n6a9X+jdMCRkNUfs4+C4vgOfCOQ5",password,account.getAddressU160().toBase58(),Base64.getDecoder().decode("0hAaO6CT+peDil9s5eoHyw=="));
                ...
                String txhash = sdk.nativevm().governance().registerCandidate(account,"0253ccfd439b29eca0fe90ca7c6eaa1f98572a054aa2d1d56e72ad96c466107a85",10000,identity.Gid,password,identity.controls.get(0).getSalt(),1,payerAcct,sdk.DEFAULT_GAS_LIMIT,0);
...
//                System.out.println("account9" +sdk.getConnect().getBalance( account9.getAddressU160().toBase58()));
            ...
            if(true) {
                System.out.println("account:" + sdk.getConnect().getBalance(account.getAddressU160().toBase58()));
                //String RESUTL =governance.getPeerInfoAll();
