    ...
    public static void main(String[] args){
        try {
            ...
            if(zptSdk.getWalletMgr().getWallet().getIdentities().size() < 1){
                identity = zptSdk.getWalletMgr().createIdentity(password);
            }else{
                identity = zptSdk.getWalletMgr().getWallet().getIdentities().get(0);
            }
            ...
            if(false) {
                Transaction tx = zptSdk.vm().makeDeployCodeTransaction(code, true, "name", "1.0", "1", "1", "1",identity.gid,0,0);
                String txHex = Helper.toHexString(tx.toArray());
                System.out.println(txHex);
                zptSdk.getConnect().sendRawTransaction(txHex);
            }
//            System.exit(0);
            String params = zptSdk.wasmvm().buildWasmContractJsonParam(new Object[]{20,30});
            System.out.println(params);
            Transaction tx = zptSdk.vm().makeInvokeCodeTransaction(zptSdk.vm().getCodeAddress(),"add",params.getBytes(),null,0,0);
            zptSdk.signTx(tx,new Account[][]{{}});
